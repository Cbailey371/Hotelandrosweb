...
@section('header', 'Editar Habitación: ' . $room->name_es)
    ...
    <div class="max-w-3xl mx-auto">
        <form action="{{ route('admin.rooms.update', $room->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div
                ...
                <div class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                        <div
                            ...
                            <img id="room-preview"
                                src="{{ $room->image ?? 'https://placehold.co/600x400?text=Sin+Imagen' }}"
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="url" name="image_url" placeholder="https://ejemplo.com/imagen.jpg"
                                    value="{{ $room->image && str_starts_with($room->image, 'http') ? $room->image : '' }}"
                ...
                <div class="border-t border-slate-200 dark:border-slate-800 pt-6">
                    ...
                    <div id="room-gallery-container" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
                        ...
                        @foreach($room->galleries as $gallery)
                            <div class="relative group aspect-square rounded-[1.5rem] overflow-hidden border-2 border-slate-100 dark:border-slate-800 shadow-sm"
                                id="gallery-item-{{ $gallery->id }}">
                                <input type="checkbox" name="gallery_ids[]" value="{{ $gallery->id }}" checked class="hidden">
                                <img src="{{ $gallery->image_path }}" class="w-full h-full object-cover">

                                <!-- Remove Button -->
                                <button type="button" onclick="removeGalleryItem('{{ $gallery->id }}')"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="text" name="name_es" value="{{ old('name_es', $room->name_es) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="name_en" value="{{ old('name_en', $room->name_en) }}" required
                            class="w-full bg-[#f0f2f5] dark:bg-slate-800 border-none rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary/50">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="description_es" rows="3"
                        class="w-full bg-[#f0f2f5] dark:bg-slate-800 border-none rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary/50">{{ old('description_es', $room->description_es) }}</textarea>
                </div>
                <div>
                    <label class="block text-sm font-bold text-[#0d141b] dark:text-white mb-2">Descripción (Inglés)</label>
                    <textarea name="description_en" rows="3"
                        class="w-full bg-[#f0f2f5] dark:bg-slate-800 border-none rounded-lg px-4 py-2 text-sm focus:ring-2 focus:ring-primary/50">{{ old('description_en', $room->description_en) }}</textarea>
                </div>
            ...
            <div class="flex justify-end gap-4">
                ...
                <button type="submit"
                    class="px-8 py-2.5 rounded-lg bg-primary text-white font-bold shadow-lg shadow-primary/20 hover:bg-primary/90 transition-all">Actualizar
                    Habitación</button>
            </div>
        </form>
    </div>
...
@push('scripts')
    <script>
        function previewMainImage(input) {
            if (input.files && input.files[0]) {
                ...
                reader.onload = function (e) {
                    ...
                    document.getElementById('main_image_id').value = ''; // Limpiar si se sube archivo nuevo
                }
        ...
        function previewNewImages(input) {
            ...
            // Limpiar previews anteriores excepto el título
